...
async function analyzeImage(imageUrl, query) {
	try {
		const apiUrl = `https://bk9.fun/ai/geminiimg?url=${encodeURIComponent(
			imageUrl,
			...
			if (citel.quoted && citel.quoted.mtype === 'conversation') {
				if (!text)
					return await citel.reply(
						`Hello ${citel.pushName}, please provide a query`,
					);
				...
				citel.quoted &&
				citel.quoted.mtype === 'imageMessage'
			) {
				if (!text)
					return await citel.reply(
						`Hello ${citel.pushName}, please provide a query for the image analysis.`,
					);
				try {
				...
				if (!text)
					return await citel.reply(
						`Hello ${citel.pushName}, how can I assist you? (Start your message with .gemini to get a reply)`,
					);

				let users = citel.sender
...
cmd(
	...
	async (Void, citel, text) => {
		if (!text)
			return await citel.reply(
				`Hello ${citel.pushName}, please provide a prompt for the magic studio. Example: .magicstudio cat`,
			);

		try {
			const apiUrl = `https://bk9.fun/ai/magicstudio?prompt=${encodeURIComponent(
				text,
			)}`;
			const response = await axios.get(apiUrl, {
