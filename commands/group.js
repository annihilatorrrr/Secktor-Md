...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (
			!text.split(' ')[0] &&
			!text.split(' ')[0].includes('whatsapp.com')
		) {
			citel.reply(
				'Link Invalid, Please Send a valid whatsapp Group Link!',
			);
		}
		let result = text
			.split(' ')[0]
	...
	async (Void, citel, text) => {
		...
		if (citel.quoted) {
			...
			if ((quoted.msg || citel.quoted).seconds > 20) {
				return citel.reply(
					'Cannot fetch videos longer than *20 Seconds*',
				);
			}
	...
	async (Void, citel, text, { isCreator }) => {
		...
		try {
			...
			if (h.length > Config.warncount) {
				teskd =
					'Removing User because Warn limit exceeded\n\n*All Warnings.*\n';
				let h = await warndb.find({
					id: citel.quoted.sender.split('@')[0] + 'warn',
				});
...
cmd(
	...
	async (Void, citel, text) => {
		...
		if (!citel.quoted.isBot) {
			...
			if (!isAdmins) {
				return citel.reply(
					'Only Admins are allowed to delete other persons message.',
				);
			}
			if (!isBotAdmins) {
				return citel.reply(
					"I can't delete anyones message without getting Admin Role.",
				);
			}
			if (!citel.quoted) {
				return citel.reply(
					`Please reply to any message. ${tlang().greet}`,
				);
			}
...
cmd(
	...
	async (Void, citel, text) => {
		...
		citel.reply(
			`*Successful Sending Broadcast To ${anu.length} Group(s)*`,
		);
	},
);
