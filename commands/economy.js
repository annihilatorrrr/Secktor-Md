...
const { sck, cmd, getBuffer, tlang, prefix } = require('../lib');
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (mongoschemas == 'false')
			return citel.reply(
				'*🚦Economy* is not active in current group.',
			);
		...
		if (daily.cd) {
			...
			return citel.reply(
				`you claimed daily ${daily.amount} 🪙 for today🎉.`,
			);
		}
	},
);
	...
	async (Void, citel, text, { isCreator }) => {
		...
		switch (value) {
			...
			case '1':
				...
				return await citel.reply(
					`*1000 🪙diamond storage has been added in ${citel.pushName} bank*`,
				);
				break;
			case '100000':
			case '2':
				...
				return await citel.reply(
					`*100000 🪙diamond storage has been added in ${citel.pushName} bank*`,
				);
			...
			case '3':
				...
				return await citel.reply(
					`*10000000 🪙diamond storage has been added in ${citel.pushName}\'s bank*`,
				);
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (deposit.noten)
			return citel.reply("You can't deposit what you don't have💰."); //if user states more than whats in his wallet
		return await citel.reply(
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (value[0] === '')
			return citel.reply(`Use ${prefix}transfer 100 @user`);
		let user = citel.mentionedJid
			? citel.mentionedJid[0]
			: citel.msg.contextInfo.participant || false;
		...
		if (a == true)
			return citel.reply(
				'you dont have sufficient money to transfer👎',
			);
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		return await Void.sendMessage(
			citel.chat,
			{
				text: `Added 📈 ${parseInt(text.split(' ')[0])} to @${
					users.split('@')[0]
				} wallet🛸.`,
				mentions: [users],
			},
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (k > balance2.wallet)
			return citel.reply(
				`*Sorry, your victim is too poor 🤷🏽‍♂️ let go🫤.*`,
			);
		let tpy = random;
		switch (random) {
			case 'ran':
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (withdraw.noten)
			return citel.reply('*🏧 Insufficient fund in bank🫤*'); //if user states more than whats in his wallet
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		citel.reply(
			media,
			{ packname: 'Secktor', author: 'Economy' },
			'sticker',
		);
		const f = [
			'up',
		...
		if (!opp)
			return citel.reply(
				'*Specify the direction you are betting on!*',
			);
		if (!gg)
			return citel.reply(
		...
		if (a == true)
			return citel.reply(
				`*Sorry ${citel.pushName}, you can only gamble with more than 🪙50.*`,
			);
		if (r == opp) {
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (
			...
			const mess5 =
				smallLose[Math.floor(Math.random() * smallLose.length)];
			if (text.split(' ')[0]) {
				...
				if (value <= balance.wallet) {
					...
					if (f1 !== f2 && f2 !== f3) {
						const deduct1 = await eco.deduct(
							user,
							secktor,
							deduff,
						);
						...
						const deduct2 = await eco.deduct(
							user,
							secktor,
							deduff,
						);
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (lol) {
			...
			let str = st
				...
				.replace(/:/g, `  `);
			return await citel.reply(
				str + `You got ${deduff * 10} in your wallet.`,
			);
		} else {
