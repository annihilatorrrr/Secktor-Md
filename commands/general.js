...
cmd(
	...
	async (Void, citel, text) => {
		if (!text) {
			return citel.reply(
				`Hey there! ${citel.pushName}. How are you doing these days?`,
			);
		}
		if (text.length < 8) {
		...
		if (!data.choices || data.choices.length === 0) {
			return citel.reply(
				'*Invalid ChatGPT API Key, Please Put New Key*',
			);
		}
...
cmd(
	...
	async (Void, citel) => {
		let { data } = await axios.get(
			'https://api.github.com/repos/SamPandey001/Secktor-Md',
		);
