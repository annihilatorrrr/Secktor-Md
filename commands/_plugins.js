...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		for (let Url of isUrl(trl)) {
			...
			return citel.reply(
				'_Plugin_ *' + l + '* _installed in Secktor._',
			);
		}
	},
);
		...
		for (let Url of urls) {
			...
			try {
				...
				try {
					...
					citel.reply(
						'_Plugin_ *' + l + '* _installed temporarily._',
					);
					...
					setTimeout(() => {
						...
						fs.unlink(tempFilePath, err => {
							if (err) {
								console.error(
									'Error deleting plugin file:',
									err,
								);
							} else {
								console.log(
									`Plugin file ${l}.js deleted successfully.`,
								);
							}
						});
