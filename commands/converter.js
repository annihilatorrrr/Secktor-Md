...
cmd(
	...
	async (Void, citel, text) => {
		...
		if (mime == 'imageMessage' || mime == 'stickerMessage') {
			...
			return citel.reply(
				'```Uhh Please, Reply To A Non Animated Sticker```',
			);
	},
);
//---------------------------------------------------------------------------
...
cmd(
	...
	async (Void, citel, text) => {
		try {
			const quot =
				citel.msg.contextInfo.quotedMessage.viewOnceMessageV2;
			if (quot) {
				if (quot.message.imageMessage) {
					console.log('Quot Entered');
					let cap = quot.message.imageMessage.caption;
					let anu = await Void.downloadAndSaveMediaMessage(
						quot.message.imageMessage,
					);
		...
		if (!citel.quoted)
			return citel.reply('```Uh Please Reply A ViewOnce Message```');
		if (citel.quoted.mtype === 'viewOnceMessage') {
			...
			if (citel.quoted.message.imageMessage) {
				...
				let anu = await Void.downloadAndSaveMediaMessage(
					citel.quoted.message.imageMessage,
				);
				...
				let anu = await Void.downloadAndSaveMediaMessage(
					citel.quoted.message.videoMessage,
				);
				return Void.sendMessage(citel.chat, {
					video: { url: anu },
					caption: cap,
...
cmd(
	...
	async (Void, citel, text) => {
		if (!citel.quoted)
			return citel.reply('Please quote/reply to any message');
		...
		return citel.reply(
			img,
			{ packname: 'Secktor', author: 'Quotely' },
			'sticker',
		);
	},
);
...
cmd(
	...
	async (Void, citel, text) => {
		...
		let fancytextt = await fancytext(
			`${text.slice(2)}`,
			text.split(' ')[0],
		);
...
cmd(
	...
	async (Void, citel, text) => {
		if (!citel.quoted)
			return citel.reply(`*Reply To any Image or video Sir.*`);
	...
	async (Void, citel, text) => {
		...
		if (mime == 'audioMessage' || mime == 'videoMessage') {
			...
			fs.unlink(media, err => {
				if (err) {
					return console.error(
						'File Not Deleted from From TOAUDIO AT : ',
						media,
						'\n while Error : ',
						err,
					);
					...
					return console.log(
						'File deleted successfully in TOAUDIO MP3 at : ',
						media,
					);
			});
		} else
