...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		if (!text)
			return citel.reply('🔍 Please provide me a valid gist url.');
		await addnote(text);
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		if (!isCreator)
			return citel.reply('This command is onlt for my Owner');
		try {
			...
			sck1.findOne({ id: users }).then(async usr => {
				if (!usr) {
					...
					if (usr.ban !== 'true')
						return citel.reply(
							`${usr.name} is already unbanned.`,
						);
					await sck1.updateOne({ id: users }, { ban: 'false' });
					return citel.reply(
						`${usr.name} is free as a bird now`,
					);
				}
			});
		} catch {
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		return citel.reply(
			`Id: ${
				text.split(' ')[0]
			}\'s note has been deleted from mongodb.`,
		);
	},
);
//---------------------------------------------------------------------------
...
cmd(
	...
	async (Void, citel, text, { isCreator }) => {
		...
		try {
			...
			if (!users)
				return citel.reply(
					`❌ Please mention any user ${tlang().greet}.`,
				);
			let pushnamer = Void.getName(users);
			sck1.findOne({ id: users }).then(async usr => {
				if (!usr) {
					...
					return citel.reply(
						`_Banned ${usr.name} from Using Commands._`,
					);
				} else {
					if (usr.ban == 'true')
						return citel.reply(
							`${pushnamer} is already Banned from Using Commands`,
						);
					await sck1.updateOne({ id: users }, { ban: 'true' });
					return citel.reply(
						`_Successfully Banned ${usr.name} from Using Commands._`,
					);
				}
			});
		} catch (e) {
...
cmd(
	...
	async (Void, citel, text, isAdmins) => {
		let alivemessage =
			Config.ALIVE_MESSAGE || `*A bot developed by SamPandey001.*`;
		const alivtxt = `
