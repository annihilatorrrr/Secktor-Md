...
cmd(
	...
	async (Void, citel, text) => {
		return await citel.reply(
			text.replace(/\+/g, String.fromCharCode(8206).repeat(4001)),
		);
	},
);
//---------------------------------------------------------------------------
...
cmd(
	...
	async (Void, citel, match) => {
		...
		const groupMetadata = citel.isGroup
			...
			: '';
		const participants = citel.isGroup
		...
		return Void.sendMessage(
			citel.chat,
			{
				text: `The most ${match} around us is *@${
					pick.split('@')[0]
				}*`,
				mentions: [pick],
			},
...
cmd(
	...
	async (Void, citel, text) => {
		if (!text)
			return citel.reply(`Example : ${prefix}fliptext Back in black`);
	...
	async (Void, citel, text, { isCreator }) => {
		...
		switch (text.split(' ')[0]) {
			case 'on':
				{
					...
					if (!checkgroup) {
						...
						return citel.reply(
							`Successfully Enabled *${tlang().title}*`,
						);
					} else {
						if (checkgroup.botenable == 'true')
							return citel.reply(
								'*Bot* was already enabled',
							);
						await sck.updateOne(
							{ id: citel.chat },
							{ botenable: 'true' },
			...
			default: {
				...
				return await Void.sendButtonText(
					citel.chat,
					buttons,
...
cmd(
	...
	async (Void, citel, text) => {
		...
		return await Void.sendButtonText(
			citel.chat,
			buttons,
			`Activate antilink: Deletes Link + kick`,
			Void.user.name,
			citel,
		);
	},
...
cmd(
	...
	async (Void, citel, text) => {
		...
		try {
			...
			let urll = `https://s.vercel.app/api?url=${
				text.match(/\bhttps?:\/\/\S+/gi)[0]
